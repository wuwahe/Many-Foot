...
from pptx_helpers import bg_rect, h_line, v_line, textbox, oval_shape, \
                          rounded_rect, add_fade_transition
...
```python
def agenda_slide(prs, sections: list, highlight_index: int = -1):
    """
...
```python
def bullet_slide(prs, title: str, bullets: list,
                 subtitle: str = '', accent_color=None):
    """
    标准要点页。bullets 支持两级缩进：以 '  '（两个空格）开头为二级要点。
    """
    SW, SH = int(prs.slide_width), int(prs.slide_height)
...
# ── paste all the slide functions above here ──
...
chart_data.add_series('2024', (100, 130, 150, 170))
chart_slide(prs, 'Revenue by Quarter', XL_CHART_TYPE.COLUMN_CLUSTERED, chart_data,
            insight='Q4 revenue hit record high',
            notes=['YoY growth 23%', 'APAC drives 40% of total'])

end_slide(prs, 'Thank You', contact='contact@acme.com')

OUT = '/workspace/output/q1-review.pptx'
prs.save(OUT)
print(f'✓ Saved: {OUT}')
```
